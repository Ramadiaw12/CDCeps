...
// GET  /api/documents/cdc/:id/format    Télécharge au format choisi (markdown/pdf)
    ...
    try {
        ...
        const htmlComplet = `
            ...
            <head>
                <meta charset="UTF-8">
                <style>
                    body { font-family: 'Arial', sans-serif; font-size: 12px; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; padding: 20px; }
                    .header { border-bottom: 3px solid #2563eb; padding-bottom: 20px; margin-bottom: 30px; }
                    .header h1 { color: #2563eb; font-size: 22px; margin: 0 0 5px 0; }
                    .header .meta { color: #666; font-size: 11px; }
                    .score-badge { display: inline-block; background: #2563eb; color: white; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: bold; margin-top: 8px; }
                    h1 { color: #2563eb; font-size: 20px; }
                    h2 { color: #1e40af; font-size: 16px; border-bottom: 1px solid #e5e7eb; padding-bottom: 5px; }
                    ...
                    li { margin: 4px 0; }
                    code { background: #f1f5f9; padding: 2px 6px; border-radius: 3px; font-size: 11px; }
                    .footer { margin-top: 40px; padding-top: 15px; border-top: 1px solid #e5e7eb; text-align: center; color: #999; font-size: 10px; }
                </style>
            </head>
            <body>
                <div class="header">
        ...
        await pool.query(
            `UPDATE cahiers_des_charges SET fichier_pdf_path = 'generated' WHERE id = $1`,
            [req.params.id]
        );
...
router.get('/cdc/:id/format', async (req, res) => {
    ...
    try {
        ...
        if (format === 'markdown') {
            ...
            return res.send(cdc.contenu_markdown);
        }
        ...
        return res.status(400).json({
            succes: false,
            message: 'Format non supporté. Utilisez "markdown" ou "pdf"'
        });

    } catch (error) {
        console.error('❌ Erreur export format:', error.message);
        return res.status(500).json({
            succes: false,
            message: 'Erreur export du CDC',
            erreur: error.message
        });
    }
});
